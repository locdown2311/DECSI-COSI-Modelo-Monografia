\section{Resultados da Classificação Heurística de Gargalos}
\label{sec:resultados_classificacao_cap}

A aplicação da árvore de decisão heurística sobre a base consolidada de 498.034 registros telemétricos permitiu quantificar a incidência de cada classe de gargalo no campus do ICEA/UFOP. Esta seção apresenta as taxas de ocorrência observadas, a carga discriminada por Access Point e os impactos diretos na vazão e na perda de pacotes.

\subsection{Tabela Geral de Ocorrência e Impacto de Gargalos}
\label{subsec:tabela_geral_gargalos}

A \autoref{tab:data_classificacao_gargalos_1} apresenta a consolidação das taxas de ocorrência e os efeitos físicos associados a cada categoria de gargalo avaliada de forma isolada ao longo do monitoramento de produção, em conformidade com os limiares estabelecidos na \autoref{sec:classificacao_limiares_cap}.
...
\end{itemize}
\section{Resultados do Agrupamento Operacional (K-Means)}
\label{sec:resultados_kmeans_cap}

Para validar de forma não-supervisionada os perfis de operação dos Access Points sem a imposição de regras manuais, aplicou-se o algoritmo de aprendizado de máquina \textit{K-Means}, conforme modelagem matemática descrita na \autoref{sec:kmeans_desenvolvimento_cap}.

\subsection{Validação e Seleção do Número Ótimo de Clusters (\texorpdfstring{$K=4$}{K=4})}
\label{subsec:validacao_kmeans_cap}

A clusterização baseou-se na aplicação do algoritmo com normalização prévia das variáveis por meio do \texttt{StandardScaler} para evitar que a diferença de escalas (como potência de sinal em dBm versus clientes em unidades) distorcesse as distâncias euclidianas. 
...
\subsection{Perfis Operacionais dos Rádios Ruckus}
\label{subsec:perfis_ruckus_cap}
...
\end{itemize}
\subsection{Dispersão Espacial dos Clusters de Access Points}
\label{subsec:dispersao_clusters_cap}

Para visualizar a distribuição e o isolamento dos clusters no espaço métrico de operação de RF, geraram-se gráficos de dispersão cruzando a Intensidade de Sinal média (eixo horizontal), o Throughput Agregado médio (eixo vertical) e as taxas médias de Retransmissão.

A \autoref{fig:cluster_ruckus_png} ilustra a dispersão dos pontos de acesso da rede Ruckus. Observa-se a separação nítida dos quatro perfis, confirmando a alta resiliência dos APs do Grupo A sob carga pesada e a caracterização dos rádios degradados por atenuação de sinal no Grupo D.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{img/cluster_ruckus.png}
  \caption{Clusterização dos Access Points Ruckus}
  \label{fig:cluster_ruckus_png}
\end{figure}

A \autoref{fig:cluster_unifi_png} apresenta a dispersão correspondente para a rede UniFi. O gráfico destaca a grande concentração de pontos operando com baixo tráfego e baixa demanda (Grupos B e D) e evidencia de forma cristalina a anomalia do AP \texttt{PICO-H-100} (Grupo C), que se posiciona isolado na região de sinal excelente com perdas crônicas por desbalanço de potência de transmissão.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{img/cluster_unifi.png}
  \caption{Clusterização dos Access Points UniFi}
