...
A fim de atestar a simultaneidade e a equivalência temporal das coletas de dados executadas nas duas infraestruturas, delimitou-se o período de monitoramento ativo de cada controladora. A Tabela 2 apresenta as datas iniciais, finais e a duração total efetiva do período amostral sob análise.
...
\IBGEtab{%
  ...
  \resizebox{0.78\textwidth}{!}{%
    \begin{tabular}{lccc}
      ...
      \midrule
      \textbf{Ruckus} & 23/06/2026 00:00 & 10/07/2026 23:55 & 14 dias úteis \\
      \textbf{UniFi} & 23/06/2026 00:02 & 10/07/2026 23:57 & 14 dias úteis \\
      \bottomrule
    \end{tabular}%
  }%
}{%
...
\textit{Nota: Ambos os datasets cobrem o mesmo período temporal (de 23/06/2026 00:00 a 10/07/2026 23:55, totalizando 14 dias úteis de monitoramento efetivo após a exclusão de finais de semana), garantindo que os cenários de carga de rede sejam diretamente comparáveis.}
...
\begin{itemize}
  ...
  \item \textbf{Ruído (Noise) e Intensidade de Sinal (IS) em Ruckus}: Alguns registros continham valores de intensidade de sinal zerados ou vazios. Onde o sinal era -100 (vazio padrão) e SNR era 0, o ruído foi definido como NaN para evitar distorções de cálculo.
...
\begin{enumerate}
  \item \textbf{Intensidade de Sinal (IS) e Ruído}: Potência medida em \textbf{dBm}.
...
\IBGEtab{%
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{lccc}
      ...
      Throughput Downstream & \texttt{tx\_bytes / uptime} & \texttt{tx\_bytes\_r * 8} & \texttt{throughput\_tx\_mbps} \\
      Throughput Upstream & \texttt{rx\_bytes / uptime} & \texttt{rx\_bytes\_r * 8} & \texttt{throughput\_rx\_mbps} \\
      Retransmissões & \texttt{tx\_retries / (tx\_pkts + tx\_retries)} & \texttt{100 - (ccq / 10)} & \texttt{retransmissoes\_pct} \\
      Volume Trafegado & \texttt{tx\_bytes}, \texttt{rx\_bytes} & \texttt{tx\_bytes\_r * uptime}, \texttt{rx\_bytes\_r * uptime} & \texttt{volume\_tx\_mb}, \texttt{volume\_rx\_mb} \\
