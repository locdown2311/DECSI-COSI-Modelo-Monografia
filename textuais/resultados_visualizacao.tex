...
\begin{itemize}
  \item \textbf{Ruckus}: Média de \texttt{-73.75 dBm}, Mediana \texttt{-75.00 dBm}.
  \item \textbf{UniFi}: Média de \texttt{-66.22 dBm}, Mediana \texttt{-67.00 dBm}.
...
\begin{itemize}
  \item O gráfico demonstra que, na população observada, os clientes UniFi apresentaram uma distribuição de sinal com menor variabilidade (concentrada em torno de -67 dBm), enquanto a Ruckus apresenta maior amplitude e dispersão das conexões ao longo do dia.
...
\begin{itemize}
  \item \textbf{2,4 GHz}: Ambas as marcas concentram o tráfego nos canais não sobrepostos \texttt{1}, \texttt{6} e \texttt{11}, validando as boas práticas de planejamento de RF adotadas no campus.
  \item \textbf{5 GHz}: A Ruckus apresenta uma alocação de espectro mais ampla e distribuída (como canais \texttt{36}, \texttt{52}, \texttt{60}, \texttt{104}, \texttt{112}, \texttt{120}, \texttt{128}, \texttt{149}), o que reduz interferência co-canal em células adjacentes.
...
\begin{itemize}
  ...
  \item \textbf{Ruckus}: Apresenta registros com cargas substancialmente maiores de clientes simultâneos, o que reflete o fato de esses equipamentos estarem implantados nas áreas de maior circulação e densidade de alunos do campus (salas de aula, auditórios e biblioteca).
...
\begin{itemize}
  \item Como esperado teoricamente no ambiente Ruckus (único fabricante com registros na banda de 5 GHz na telemetria coletada), a faixa de \textbf{5 GHz} oferece vazão física superior, superando a banda de \textbf{2,4 GHz} que possui limitações de largura de canal e maior concorrência de meio.
...
\begin{itemize}
  \item A banda de \textbf{2,4 GHz} exibe taxas médias superiores de retransmissão, decorrentes do maior congestionamento espectral e das colisões de quadros provocadas pela contenção CSMA/CA no reduzido número de canais não-sobrepostos disponíveis.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item No ambiente Ruckus, o pico de clientes únicos em uma única hora ocorreu no dia \textbf{02/07} às \textbf{20:00}, registrando \textbf{522} dispositivos.
    \item No ambiente UniFi, o pico de clientes únicos ocorreu no dia \textbf{01/07} às \textbf{18:00}, registrando \textbf{106} dispositivos.
\end{itemize}
